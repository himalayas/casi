 ...
 * After pool is initialized, a client will request a com.casi.demo.hash.SockIO object by calling getSock with the cache key<br/>
 * The client must always close the com.casi.demo.hash.SockIO object when finished, which will return the connection back to the pool.<br/>
 * <h3>An example of retrieving a com.casi.demo.hash.SockIO object:</h3>
 * <pre>
 *		SockIOPool.com.casi.demo.hash.SockIO sock = SockIOPool.getInstance().getSock( key );
 *		try {
 *			sock.write( "version\r\n" );	
 *			sock.flush();	
...
public class SockIOPool {
	...
	/** 
	 * Creates a new com.casi.demo.hash.SockIO obj for the given server.
	 *
	 ...
	 * @return com.casi.demo.hash.SockIO obj or null if failed to create
	 */
	@SuppressWarnings("unchecked")
	protected SockIO createSocket( String host ) {
		...
		try {
			...
			if ( !socket.isConnected() ) {
				log.error( "++++ failed to get com.casi.demo.hash.SockIO obj for: " + host + " -- new socket is not connected" );
				try {
					socket.trueClose();
				}
				catch ( Exception ex ) {
					log.error( "++++ failed to close com.casi.demo.hash.SockIO obj for server: " + host );
			...
			log.error( "++++ failed to get com.casi.demo.hash.SockIO obj for: " + host );
			log.error( ex.getMessage(), ex );
			socket = null;
		}
	...
	/** 
	 * Returns appropriate com.casi.demo.hash.SockIO object given
	 * string cache key.
	 * 
	 * @param key hashcode for cache key
	 * @return com.casi.demo.hash.SockIO obj connected to server
	 */
	 ...
	 *
	 * Trys to get com.casi.demo.hash.SockIO from pool.  Fails over
	...
	public SockIO getSock( String key, Integer hashCode ) {
		...
		if ( !this.initialized ) {
			log.error( "attempting to get com.casi.demo.hash.SockIO from uninitialized pool!" );
			return null;
		}
	...
	/** 
	 * Returns a com.casi.demo.hash.SockIO object from the pool for the passed in host.
	 *
	 ...
	 * @return com.casi.demo.hash.SockIO object or null if fail to retrieve one
	 */
	@SuppressWarnings("unchecked")
	public synchronized SockIO getConnection( String host ) {
		...
		log.debug( "++++ creating " + create + " new com.casi.demo.hash.SockIO objects" );
	...
	/** 
	 * Checks a com.casi.demo.hash.SockIO object in with the pool.
	 *
	 ...
	 *
	 * This is called from com.casi.demo.hash.SockIO.close().  Calling this method<br/>
	 * directly without closing the com.casi.demo.hash.SockIO object first<br/>
	...
	protected synchronized void selfMaint() {
		...
		for ( Iterator i = availPool.keySet().iterator(); i.hasNext(); ) {
			...
			// reset the shift value for creating new com.casi.demo.hash.SockIO objects
