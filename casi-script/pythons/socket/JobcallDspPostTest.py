...
def httpclient(timeout):
    socket.setdefaulttimeout(int(timeout))
    i=random.Random().randint(1,100000)
    id="kevin_%s"%i
    url = "http://10.224.82.73:1984/task"
    data = {"callbackURL": "http://nsz2nbrwss.qa.webex.com/nbr/CallbackServlet?action=standardNBRReady",
    ...
    fd = None
    try:
        fd = urllib2.urlopen(url=url, data=json.dumps(data))
